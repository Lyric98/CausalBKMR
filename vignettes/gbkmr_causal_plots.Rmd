...
[`tools/generate-causal-plot-examples.R`](https://github.com/Lyric-o/CausalBKMR/blob/master/tools/generate-causal-plot-examples.R).
...
the median. Every curve uses the same reference intervention: the focal
exposure is at its 25th percentile and the conditional exposure is at its
median:

\[
E\{Y(A_j=x, A_k=c, A_{-(j,k)}=0.50)\}
- E\{Y(A_j=q_{0.25,j}, A_k=q_{0.50,k}, A_{-(j,k)}=0.50)\}.
\]

The direction of each pair matters: the focal exposure is on the horizontal axis,
and the conditional exposure defines the colored levels. Using a common
reference allows the three curves to remain vertically separated when the
conditional exposure changes the counterfactual outcome. Set `reference` and
`conditional_reference` to customize the common intervention.
